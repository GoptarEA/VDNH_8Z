...
var dict = new Map();
s.split("\n").forEach(elem => {
    dict.set(elem.split(" - ")[1], [parseFloat(elem.split(" - ")[0].split(', ')[0]), 
    parseFloat(elem.split(" - ")[0].split(', ')[1])]);
});

var rests = new Map();
s.split("\n").forEach(elem => {
    if (elem.split(" - ")[1].toLowerCase().includes("ресторан") || elem.split(" - ")[1].toLowerCase().includes("кафе")) {
        rests.set(elem.split(" - ")[1], [parseFloat(elem.split(" - ")[0].split(', ')[0]), 
        parseFloat(elem.split(" - ")[0].split(', ')[1])]);
    }
});
...
var museums = new Map();
s.split("\n").forEach(elem => {
    if (elem.split(" - ")[1].toLowerCase().includes("музей") || elem.split(" - ")[1].toLowerCase().includes("павильон")) {
        museums.set(elem.split(" - ")[1], [parseFloat(elem.split(" - ")[0].split(', ')[0]), 
        parseFloat(elem.split(" - ")[0].split(', ')[1])]);
    }
});

var restsCoords = {pointCoords: function (points) { return points.map(elem => rests.get(elem)); }};
var museumsCoords = {pointCoords: function (points) { return points.map(elem => museums.get(elem)); }};
var Coords = {pointCoords: function (points) { return points.map(elem => dict.get(elem)); }};
